...
cumulative_contribution_ratios = contribution_ratios.cumsum()  # cumsum() で寄与度の累積和を計算
cont_cumcont_ratios = pd.concat(
    [contribution_ratios, cumulative_contribution_ratios],
    axis=1).T
cont_cumcont_ratios.index = ['contribution_ratio', 'cumlative_contribution_ratio']  # 行の名前を変更
cont_cumcont_ratios.to_csv('pca_cont_cumcont_ratios.csv')

# %% 寄与率を棒グラフで、累積寄与率を線で入れたプロット図を重ねて描画
x_axis = range(1, contribution_ratios.shape[0] + 1)  # 1 から成分数までの変数が x 軸の値
plt.rcParams['font.size'] = 18
...
plt.ylabel('Contribution ration(blue),\nCumulative contribution ration(red)')  # 縦軸の名前。\n で改行しています
plt.show()
